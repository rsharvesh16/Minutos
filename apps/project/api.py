...
def api_stop_timer(request):
    ...
    tracked_minutes = int((datetime.now(pytz.timezone('Asia/Kolkata')) - entry.created_at).total_seconds() / 60)
...
def api_get_tasks(request):
    ...
    if project_id:
        ...
        t = Task.objects.get(team=team,project=project,Assign=request.user.pk)
        tasks.append(
