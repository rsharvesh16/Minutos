...
def projects(request):
    ...
    # auser = str(task.Assign)
    admin = str(team.created_by)
    ...
    return render(request, 'project/projects.html', {'team': team, 'projects': projects,'admin':admin,})
...
def project(request, project_id):
    ...
    return render(request, 'project/project.html', {'team': team, 'project': project, 'tasks_todo': tasks_todo, 'tasks_done': tasks_done,'admin':admin})
...
def task(request, project_id, task_id):
    ...
    return render(request, 'project/task.html', {'today': datetime.today(), 'team': team, 'project': project, 'task': task, 'admin':admin, 'auser':auser})
...
def edit_task(request, project_id, task_id):
    ...
    act_user = str(task.Assign)
    admin = str(task.created_by)

    get_team_lst = Team.objects.get(pk=request.user.userprofile.active_team_id)

    mem_names = []
    for name in get_team_lst.members.all():
        mem_names.append(name.username)

    mem_lst_new = []

    if act_user in mem_names:
        mem_names.remove(act_user)
    
    # if str(team.created_by) in mem_names:
    #     mem_names.remove(str(team.created_by))

    if request.method == 'POST':
        assigned_user = request.POST.get('users')
        title = request.POST.get('title')
        status = request.POST.get('status')

        get_user = User.objects.get(username=assigned_user)

        if title:
            task.title = title
            task.status = status
            task.Assign = get_user
            task.save()
    ...
    return render(request, 'project/edit_task.html', {'team': team, 'project': project, 'task': task,  'memlst':mem_names, 'active':act_user, 'admin':admin})
...
def track_entry(request, entry_id):
    ...
    # tsk = Task.objects.get(project=entry.project.pk,title=entry.task)
